...
export default function AdminDashboard() {
  ...
  async function loadDashboardData() {
    ...
    const totalRevenue = ordersRes.data?.reduce((sum: number, order: any) => sum + order.amount, 0) || 0
    setStats({
      ...
      monthlyRevenue: calculateMonthlyRevenue(ordersRes.data || [])
    })
  }
...
function UsersTab() {
  ...
  async function loadUsers() {
    ...
    const [{ data: profiles }, { data: userList }] = await Promise.all([
      supabase.from('user_profiles').select('user_id, full_name, company_name, is_admin'),
  ...
  async function toggleAdmin(userId: string, currentStatus: boolean) {
    const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!)
    await supabase.from('user_profiles').update({ is_admin: !currentStatus }).eq('user_id', userId)
